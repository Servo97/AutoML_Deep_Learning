...
def generate_splits(df, target, is_train=True, split=0.25, tgt_type=''):
    if is_train:
        ...
        if tgt_type!='regression':
            ...
            y = enc_df
            pickle.dump(enc, open('./model_info/OHE.pkl','wb'))
        else:
            y = df[target]
        ...
        try:
            ...
            if tgt_type!='regression':
                ...
                enc_df = pd.DataFrame(enc.transform(df[[target]]).toarray())
                y = enc_df
            else:
                y = df[target]
...
def preprocess_csv(file_path, target):
    ...
    for column in df:
        if column.lower() in ["id", 'index', 'date', 'timestamp', 'time', 'cell_id']:
...
def preprocess_csv_test(file_path, target, tgt_type):
    ...
    df[real_valued_cols] = scaler.transform(df[real_valued_cols])
    if tgt_type=="regression":
        scaler_t = pickle.load(open('./model_info/scaler_t.pkl','rb'))
        df[[target]] = scaler_t.transform(df[[target]])
    else:
        pass
    return df
...
def read_and_preprocess_test(file_path,target, tgt_type):
    ...
    X,y = generate_splits(df, target, is_train=False, tgt_type = tgt_type)
    return X, y

def read_and_preprocess(file_path,target,split):
    df, tgt_type = preprocess_csv(file_path, target)
    x_train, x_val, y_train, y_val = generate_splits(df, target, True, split=split, tgt_type=tgt_type)
